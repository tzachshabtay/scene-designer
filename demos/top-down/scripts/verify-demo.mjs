...
const expectedRoofIds = new Set([
  "forest-roof-moss-cottage",
  "forest-roof-river-lodge"
]);
const seenRoofIds = new Set();
for (const object of roofObjects) {
  assert(expectedRoofIds.has(object.id), `${object.id} is not a recognized cottage roof.`);
...
assert(seenRoofIds.size === expectedRoofIds.size, "Every cottage must have one recognized roof.");
